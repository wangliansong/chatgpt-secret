...
import com.secret.chatgpt.front.api.parser.ChatCompletionResponseParser;
import com.secret.chatgpt.front.api.storage.ApiKeyDatabaseDataStorage;
...
public class ApiKeyResponseEmitter implements ResponseEmitter {
    ...
    private void addContextChatMessage(ChatMessageDO chatMessageDO, LinkedList<Message> messages) {
        //TODO_WANGLS 注释：防止消息递归数太多，消耗tokens
        if (messages.size() >= 4){
            return;
        }
