package com.secret.chatgpt.front.api.storage;
...
import com.secret.chatgpt.base.enums.ChatMessageStatusEnum;
import com.secret.chatgpt.base.enums.ChatMessageTypeEnum;
...
public abstract class AbstractDatabaseDataStorage implements DataStorage {
    ...
    @Resource
    protected UserMemberRelationService userMemberRelationService;
    ...
    public void onComplete(ChatMessageStorage chatMessageStorage) {
        ...
        //TODO_WANGLS 注释:套餐使用次数加一
        Long userId = questionChatMessageDO.getUserId();
        UserMemberRelationDO userMemberRelationDO = userMemberRelationService.selectByUserId(userId);
        Long useChatNum = userMemberRelationDO.getUseChatNum();

        userMemberRelationDO.setUseChatNum(useChatNum + 1);
        userMemberRelationDO.setUpdatedTime(new Date());
        userMemberRelationService.updateById(userMemberRelationDO);
    }
